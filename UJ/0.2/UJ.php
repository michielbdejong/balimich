...
class UnhostedJSONParser {
	...
	private $fields = array(
		'UJ/0.2' => array(
		      ...
		      'MSG.RECEIVE' => array('keyPath' => true, 'delete' => true, 'limit' => true, 'pubPass' => true),
		                    //
		      'ACCT.CREATE' => array('creationToken' => true, 'pubPass' => true, 'subPass' => true),
		'ACCT.GIVEPOPSHAKE' => array('creationToken' => true, 'pubPass' => true, 'toApp' => true),
		   'ACCT.DISAPPEAR' => array('pubPass' => true),
		    'ACCT.GETSTATE' => array('pubPass' => true),
		                    //
		      'MIGR.EXPORT' => array('toUser' => true, 'toNode' => true, 'migrationToken' => true),
		      'MIGR.IMPORT' => array('fromUser' => true, 'fromNode' => true, 'migrationToken' => true),
		     'MIGR.MIGRATE' => array('migrationToken' => true),
		    'MIGR.MIGRATED' => array('migrationToken' => true),
		                    ),
		);
	...
	function checkFields($POST) {
		...
		foreach($POST as $fieldName => $fieldValue) {
			if($fieldName != 'protocol' && $fieldName != 'action' && $fieldName != 'user') {
	...
	function parse() {
		...
		switch($action) {
			case 'KV.GET' : 
				list($accountId, $partition) = Accounts::getAccountId($params['POST']['user'], $params['node'], $params['app'], $params['POST']['subPass'], false);
				return KeyValue::get($accountId, $partition, $params['POST']['keyPath']);
			case 'KV.SET' : 
				list($accountId, $partition) = Accounts::getAccountId($params['POST']['user'], $params['node'], $params['app'], $params['POST']['pubPass'], true);
			...
			case 'ACCT.CREATE' : 
				return Accounts::create($params['POST']['user'], $params['node'], $params['app'], $params['POST']['creationToken'], $params['POST']['pubPass'], $params['POST']['subPass']);
			case 'ACCT.GIVEPOPSHAKE' : 
				//this call is only here to throw exceptions as appropriate:
				Accounts::getAccountId($params['POST']['user'], $params['node'], $params['app'], $params['POST']['pubPass'], true);
				return Accounts::givePopShake($params['POST']['user'], $params['node'], $params['POST']['toApp'], $params['POST']['creationToken'], $params['app']);
			case 'ACCT.DISAPPEAR' : 
				list($accountId, $partition) = Accounts::getAccountId($params['POST']['user'], $params['node'], $params['app'], $params['POST']['pubPass'], true);
